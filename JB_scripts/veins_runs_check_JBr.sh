...
if [ "$fileRSUCheck" == '' ]; then
    ...
    for lineFile in $fileRSUCheck; do # Read the fileRSUCheck line by line and set in lineFile
        ...
        if grep -q "This simulation run terminated correctly" < "$lineFile"; then # Test if terminated success
    ...
    sizeResultFile=$(du "$tmpFilePathCorrectly" | awk '{print $1}') # Get the size of result error file
    ...
    sizeResultFile=$(du "$tmpFilePathError" | awk '{print $1}')
