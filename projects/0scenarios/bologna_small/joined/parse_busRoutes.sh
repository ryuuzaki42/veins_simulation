...
# until depart=1260. For example: f(2)=f(3)=1, f(5)=2
#
...
if [ "$fileInput" == '' ]; then
    ...
    fileOutput=`echo "$fileInput" | rev | cut -d "." -f2- | rev`_parsed.xml

    echo -e "\nThis script parse value of depart time"
    echo -e "fileInput: $fileInput\nfileOutput: $fileOutput"
    echo "The parse is f(x)= integerPart(x/2), e.g., f(2)=f(3)=1 until depart=1260"
    ...
    if [ "$continueOrNot" == 'y' ]; then
        ...
        inValue=0
                    # (10.5 min + 10.5 min) + 1 s
        while [ "$inValue" -lt 1261 ]; do
            outValue=`echo "$inValue/2" | bc`
